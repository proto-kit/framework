  ...
  public zkProgramFactory() {
    const program = ZkProgram({
      ...
      methods: {
        foo: {
          privateInputs: [Field],
          method: (async (publicInput: TestPublicInput, bar: Field) => {
            return this.foo(publicInput, bar);
          }).bind(this),
        },

        fail: {
          privateInputs: [],
          method: (async (publicInput: TestPublicInput) => {
            return this.fail(publicInput);
          }).bind(this),
        },
      },
    });
    ...
    testCases.push([
      true,
      {
        verificationKey: {
          ...
          hash: Field(0), // TODO determine correctly
        },
